...
if exist ".venv\Scripts\python.exe" (
  ...
  if errorlevel 1 (
    echo [WARN] Existing backend virtual environment is broken. Recreating .venv...
    rmdir /s /q ".venv"
  )
)
...
REM --------------------------------------------------
set "PYI_TMP=%~dp0backend\.pyi_tmp"
set "TMP=%PYI_TMP%"
set "TEMP=%PYI_TMP%"
set "TMPDIR=%PYI_TMP%"
...
REM Runtime directories and input link policy.
REM This BAT is designed for normal double-click startup without administrator rights.
REM hardlink is preferred because it does not copy large input files and does not need admin.
REM symlink is disabled by default because Windows usually requires Developer Mode or admin.
REM --------------------------------------------------
set "LOCAL_WEB_RUNTIME_DIR=%~dp0backend\runtime_user"
set "LOCAL_WEB_INPUT_LINK_ORDER=hardlink"
set "LOCAL_WEB_ALLOW_INPUT_HARDLINKS=1"
set "LOCAL_WEB_ALLOW_INPUT_SYMLINKS=0"
set "LOCAL_WEB_INPUT_LINK_FALLBACK=error"
set "LOCAL_WEB_MODEL_REUSE_LARGE_RESOURCE_GROUP_CAP_GB=2.0"
set "LOCAL_WEB_PARALLEL_PROGRESS_SCAN_SECONDS=1.0"
set "LOCAL_WEB_PARALLEL_MONITOR_LOG_SECONDS=20"
set "LOCAL_WEB_DYNAMIC_WORKER_BOOST=1"
set "LOCAL_WEB_DYNAMIC_WORKER_BOOST_EXTRA=1"
set "LOCAL_WEB_DYNAMIC_WORKER_BOOST_CPU_BELOW=55"
set "LOCAL_WEB_DYNAMIC_WORKER_BOOST_MEMORY_BELOW=88"
set "LOCAL_WEB_DYNAMIC_WORKER_BOOST_MIN_MEMORY_GB=2.0"
if not exist "%LOCAL_WEB_RUNTIME_DIR%" mkdir "%LOCAL_WEB_RUNTIME_DIR%"
